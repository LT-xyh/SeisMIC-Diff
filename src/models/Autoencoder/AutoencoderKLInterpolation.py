...
class AutoencoderKLInterpolation(nn.Module):
    """
    Wrap ``diffusers.AutoencoderKL`` with resize layers that match the project shape.

    Expected flow:
        [1, 70, 70] -> init_conv -> [16, 64, 64] -> AutoencoderKL -> [16, 16, 16]
        [16, 16, 16] -> AutoencoderKL decoder -> [16, 64, 64] -> final_conv -> [1, 70, 70]
    """
    ...
    def __init__(self, latent_channels=16, depth_vel_shape=(1, 70, 70), depth_vel_reshape=(16, 64, 64),
        ...
        self.autoencoder_kl = AutoencoderKL(
            in_channels=depth_vel_reshape[0],
            out_channels=depth_vel_reshape[0],
            down_block_types=down_block_types,
            up_block_types=up_block_types,
            block_out_channels=block_out_channels,
            latent_channels=latent_channels,
            force_upcast=False,  # Keep mixed-precision training enabled when possible.
        )
...
def test_autoencoder_kl_mlp():
    ...
    input_tensor = torch.randn(3, 1, 70, 70)
